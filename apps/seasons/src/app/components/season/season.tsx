...
const Icon = (props: { className: string }) => (
  <i className={`${props.className} icon massive`} />
);
...
export const Season = (props: SeasonDisplayProps) => {
  ...
  return (
    <div className={`${bem} ${bem}--${season}`}>
      <Icon
        data-testid={`${iconName}-left`}
        className={`${bem}__icon ${bem}__icon--left ${iconName}`}
      />
      <p className={`${bem}__message`}>{text}</p>
      <Icon
        data-testid={`${iconName}-right`}
        className={`${bem}__icon ${bem}__icon--right ${iconName}`}
      />
    </div>
  );
};
